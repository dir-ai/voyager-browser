...
const TOOLS = [
  {
    name: 'observe_page',
    description:
      "Read-only observation of ONE live web page: title/structure, forms (with insecure/cross-origin/sensitive flags), links (external + unsafe target=_blank), script origins, security posture (HTTPS/HSTS/CSP/mixed-content/cookies incl. SameSite/CSRF + CORS wildcard & the dangerous reflected-origin+credentials combo) and accessibility signals (lang, img alt coverage, heading order) → findings with DESCRIBED (never applied) fixes. ALSO runs read-only leak detectors: body-content signatures (directory listings, language-specific stack traces, verbose framework debug/error pages), exposed JWTs decoded from the HTML/headers/cookies/same-origin bundles (flags alg:none, expired, and no-exp — NO signature verification, NO secret cracking), HARDCODED SECRETS scanned in same-origin JS bundles (Stripe/OpenAI sk-, AWS AKIA, GitHub ghp_/PAT, Slack, Google, PEM private keys, generic high-entropy assignments — reported REDACTED, never the raw value), and PASSIVE DISCOVERY of a short fixed list of well-known sensitive paths (/.git/config, /.env, /.svn/entries, /.DS_Store, /config.json, /wp-config.php~, /backup/, /uploads/) via bounded same-origin GETs that flag only a CONFIRMED signature (never a bare 200). When authorized:true AND a GraphQL endpoint (/graphql, /api/graphql, or a discovered one) is present, sends ONE READ-ONLY introspection query (a POST to your own endpoint) and flags graphql-introspection-enabled with the exposed types/fields; without authorized it is only suggested, never sent. Parses STATIC HTML only — no JavaScript execution, no headless browser, so it cannot see client-side-rendered runtime state. All target-derived text is returned FRAMED as untrusted. SSRF-gated: refuses non-http(s) URLs and, by default, anything resolving to private/loopback/cloud-metadata addresses — pass authorized:true to observe your OWN private/loopback/intranet target (cloud-metadata + link-local stay blocked). NEVER mutates, submits a form, or exploits — the only non-GET is the authorized read-only GraphQL introspection query. isError:true means the page could not be observed, not that it is clean.",
    inputSchema: {
      type: 'object',
